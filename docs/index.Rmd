...
author: "Jacob Hilse, Megan Chiang, Vincent Kao, Xuan Yi"
date: "11/14/2022"
...
Changed & Reworded Research Questions from P01

Added 2 new datasets education.csv & unemployment.csv

Late commit to make table look better (talked to Shiina for permission)
...
num_observations <- summary_info$num_observations
adv_1995 <- summary_info$all_adv_1995
adv_2005 <- summary_info$all_adv_2005
adv_2015 <- summary_info$all_adv_2015
high_col_yr <- summary_info$col_max_year
```

One of our dataset includes the highest level of education for different gender and age groups through 2 decades from 1995-2015. The dataset is categorized into `r num_observations` ways. We can see that more people are graduating from college and earned advanced degrees. As only `r adv_1995`% of people had it in 1995, and that number rose to `r adv_2005`% in 2005 and `r adv_2015`% in 2015. Also, the proportion of highest level of education and college graduates also rose through the years, as `r high_col_yr` saw the highest percentage in people who graduated from college.

### Table

```{r, echo = FALSE}
# Summary Info Table
source("../source/table_summary_info.R")
HEtable
```

From the table, we can see that the amount of college graduates steadily rose through time for both males and females. We included the table so that we can visually compare and add to the findings in our summary info. More people have been to college in 2015 compared to 1995, from about 35% to around 40%. The change is not too drastic, but is noticeable enough through time and should help us investigate further into education and demographics further down the line.
...
From this chart, we can compare the similarities and differences for the highest level of education based on gender. The dataset isn't all inclusive, as it only included male and female. However, we can still make some observations and discover inequality amongst gender and education. The data shows that more males have the highest levels of educations like Doctorates and professional school. While the proportions for both males and females that has a High School diploma or lower is closer to each other.
...
We included this chart to have a better understanding of how people's highest level of education has changed over the last couple of decades. We are able to see some trends on this graph. While the largest proportion of people still only has a High School diploma as their highest level of education, we can see that more and more people are going to and graduating from college, and even earning more advanced degrees.
...
We included this chart to see the relationship between the adults who has a college degree and the unemployment rate for people living in different parts of America as we hoped that it will help us understand how education affects employment for different demographics in America. While the data does not provide a strong proof to the two variables having a direct relationship, we discovered that areas with the most adults having a Bachelor's degree is cities, followed by suburbs, towns, then rural areas. One possibility for this trend is that because more jobs in cities are more technical and require a college degree than those in more rural places.
